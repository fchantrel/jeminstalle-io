...
public class DataProServiceImpl implements DataProService {
	...
	public DataPro getDataPro(String activite, String ou) {
		...
		try {
			...
			if (lstRefGeo.size() > 0) {
				...
				System.out.println("Pas de lieu trouve pour le code insee : "
						+ ou);
		...
		/**
		 * String departement = refGeo.getZipcode().substring(0, 2); Pollution
		 * pollution = pollutionDAO.findByNodepartement(departement).get(0);
		 * Couverture4G couverture4G =
		 * couverture4GDAO.findByCodedepartement(departement); Ensoleillement
		 ...
		 * precipitationDAO.findByNodepartement(departement); Nucleaire
		 * nucleaire = nucleaireDAO.findByNumdep(departement);
		 */

		// proDAO.findByCoordonneesAndRubrique(String.valueOf(refGeo.getLatitude()),
		// String.valueOf(refGeo.getLongitude()), "restaurant");
	...
	public DataPro getDataProByRegion(String activite, String ou) {
		...
		try {
			...
			int nbProParActiviteParRegion = proDAO.findByRegionAndActivite(ou,
					activite, "1");

			dp.setNbPro(nbProParActiviteParRegion);
			if (dp.getPopulation() > 0) {
				double ratio = ((double) (nbProParActiviteParRegion))
						/ ((double) dp.getPopulation()) * 1000;
				dp.setRatio(ratio);
			} else {
				dp.setRatio(0.123);
			}
	...
	public DataPro getDataProByDepartement(String activite, String ou) {
		...
		try {
			...
			int nbProParActiviteParDepartement = proDAO
					.findByDepartementAndActivite(ou, activite, "1");

			dp.setNbPro(nbProParActiviteParDepartement);
			if (dp.getPopulation() > 0) {
				double ratio = ((double) (nbProParActiviteParDepartement))
						/ ((double) dp.getPopulation()) * 1000;
				dp.setRatio(ratio);
			} else {
				dp.setRatio(0.123);
	...
	public DataPro getDataProByCommune(String activite, String ou) {
		...
		try {
			...
			int nbProParActiviteParCommune = proDAO.findByCommuneAndActivite(
					ouTrouve, activite, "1");

			dp.setNbPro(nbProParActiviteParCommune);
			if (dp.getPopulation() > 0) {
				double ratio = ((double) (nbProParActiviteParCommune))
						/ ((double) dp.getPopulation()) * 1000;
				dp.setRatio(ratio);
			} else {
				dp.setRatio(0.123);
